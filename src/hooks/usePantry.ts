...
import { validate_plain_obj, validate_arr } from "../utils/misc.ts"
...
const getProvider = async ({ project, parseYaml }: { project: string, parseYaml: (file: Path) => Promise<unknown> }) => {
  for (const prefix of pantry_paths()) {
    ...
    const yaml = validate_plain_obj(await parseYaml(filename))
...
function entry({ project }: { project: string }): Entry {
  for (const prefix of pantry_paths()) {
    ...
    const yml = async () => {
      try {
        const yml = await parseYaml(filename)
...
function tokenizePackage(pkg: Package) {
  return [{ from: "prefix", to: usePrefix().join(pkg.project, `v${pkg.version}`).string }]
}
